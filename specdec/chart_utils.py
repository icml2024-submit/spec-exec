...
def get_frontier(input, size="tree_size", util="num_gen"):
    
    if isinstance(input, LogReader):
        dfa = input.dfa
    else:
        dfa = input
    pts = dfa.loc[:, [size, util]].values
    hull = ConvexHull(pts)
    pts2 = pts[hull.vertices]
    ...
    result = np.stack(pts3)
    result = pd.DataFrame(result, columns=[size, util])
...
class LogReader:
    ...
    def __init__(self, save_name, log_dir="./logs", name=None, groupby_columns=None, filter_dict=None, n_runs=100500):
        """
        ...
        for sname in save_name:
            file_path = f"{log_dir}/{sname}.jsonl"
    ...
    def get_frontier(self, budget="tree_size", util="num_gen"):
        """
